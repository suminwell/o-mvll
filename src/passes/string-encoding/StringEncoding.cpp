...
bool StringEncoding::processGlobal(Use &Op, GlobalVariable &G,
  ...
  genRoutines(Triple{StringRef(TargetTriple)}, EI, Ctx);
  ...
  unsigned GlobalIDHashVal = xxh3_64bits((G.getName() + Twine(':') + Twine(static_cast<unsigned>(G.getLinkage()))).str());
...
bool StringEncoding::processLocal(Instruction &I, Use &Op, GlobalVariable &G,
  ...
  std::string TargetTriple = I.getModule()->getTargetTriple().str();
  genRoutines(Triple{StringRef(TargetTriple)}, EI, Ctx);
  EI.EncodeFn(Encoded.data(), Str.data(), Key, StrSz);
